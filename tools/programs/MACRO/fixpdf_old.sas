...
%macro fixpdf(inpath=, anaspec=);	
	...
	%let outFile=define_postprocessed.pdf; *** subject to change ***;
...
/*		file "fixpdf.bat";*/
	...
	*** Create default for OUTXML and make a backup before updating ***; 
	...
	%let mvcmd=no;
   	%let backup=no;
	data _null_;
        curdate = "_" || compress(tranwrd(strip(tranwrd(put(date(),yymmdd10.) || '_' || put(time(),time5.),':','_')),'-',''));
        inbackup = tranwrd("&inFile",'.pdf','_preprocessed'||strip(curdate) || '.pdf');
		outbackup = tranwrd("&outFile",'.pdf',strip(curdate) || '.pdf');
